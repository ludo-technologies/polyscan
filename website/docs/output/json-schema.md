...
For Go, Rust and C++, `risk_level` is derived from the complexity with flat dispatch collapsed rather than from `complexity` itself. A `switch` or `match` that holds no decision point beyond its own arms counts as a single decision point for that purpose, because its arm count measures the width of a lookup table rather than branching logic. This is the shape of a key handler or a state dispatcher, where every arm delegates to a handler and returns. A construct with an arm that branches, loops, guards or holds a nested switch keeps its full arm count. The arms cyclomatic complexity does not count are read the same way, so branching in the `default` of a Go or C++ switch or in the last arm of a Rust match also keeps the full arm count. The reported `complexity` is never changed by this, so it stays comparable with gocyclo and the other cyclomatic complexity tools.

The short-circuit operators of one returned expression are collapsed the same way, so `return a && b && c && d` counts as a single decision point for `risk_level`. The operands of a boolean predicate decide the value the function hands back rather than the path it takes to its one exit. Operators in a condition, in an assignment, and in a returned expression that holds a branch of another kind, such as a ternary or a closure that branches, are counted one by one. Each value of a multi-value Go return is an expression of its own, and the statements inside a closure or lambda body belong to the function that holds it, not to the expression the closure sits in, so only what the closure itself returns is collapsed with it. A Rust function returns the tail expression of its body, and a closure its tail or its whole expression body, while the tail of an inner block is not a return. Here too the reported `complexity` is never changed.
