...
For Go, Rust and C++, `risk_level` is derived from the complexity with flat dispatch collapsed rather than from `complexity` itself. A `switch` or `match` whose every arm holds no decision point of its own counts as a single decision point for that purpose, because its arm count measures the width of a lookup table rather than branching logic. This is the shape of a key handler or a state dispatcher, where every arm delegates to a handler and returns. A construct with an arm that branches, loops, guards or holds a nested switch keeps its full arm count. The reported `complexity` is never changed by this, so it stays comparable with gocyclo and the other cyclomatic complexity tools.
