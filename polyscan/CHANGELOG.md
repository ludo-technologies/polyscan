...
- The Go, Rust and C++ risk level is derived from the complexity with flat dispatch collapsed. A `switch` or `match` whose every arm holds no decision point of its own counts as a single decision point, because its arm count measures the width of a lookup table rather than branching logic; an arm that branches, loops, guards or holds a nested switch keeps the whole construct counted arm by arm. The reported `complexity` is unchanged and stays comparable with gocyclo. Before, a Bubble Tea or tview key handler whose every arm delegates to a method reached medium or high risk on its arm count alone (#142)
